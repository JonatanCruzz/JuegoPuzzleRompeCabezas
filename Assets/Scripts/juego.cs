...
public class juego : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                if (!hit.transform.GetComponent<pieza>().piezaEncajada)
                {
                    PiezaSeleccionada = hit.transform.gameObject;
                    PiezaSeleccionada.GetComponent<pieza>().piezaSeleccionada = true;
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            {
                PiezaSeleccionada.GetComponent<pieza>().piezaSeleccionada = false;
